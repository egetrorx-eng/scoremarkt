...
export default function SocialTape() {
    const [events, setEvents] = useState<TapeEvent[]>([]);

    // Mock live feed generation
    useEffect(() => {
        const initialEvents: TapeEvent[] = [
            { id: '1', type: 'WHALE_BUY', user: 'AlphaTerminal_01', content: 'Placed $1.2k on Soccer: Arsenal Win', timestamp: Date.now() - 5000, amount: '$1,200' },
            { id: '2', type: 'ALPHA_SIGNAL', user: 'ScoreMarkt AI', content: 'High Alpha (92%) detected on Champions League', timestamp: Date.now() - 15000 },
            { id: '3', type: 'WHALE_BUY', user: 'LiquidityHunter', content: 'Positioned $850 on Underdog Victory (Serie A)', timestamp: Date.now() - 30000, amount: '$850' },
        ];
    ...
    return (
        <div className="card p-6 border border-white/10 bg-midnight-800/40 backdrop-blur-md overflow-hidden relative group">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[8px] font-bold text-lime-500/60 uppercase tracking-widest">Live Signals</span>
                </div>
            </div>
